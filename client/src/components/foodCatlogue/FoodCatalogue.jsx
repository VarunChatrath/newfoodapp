...
import classes from './foodcatalogue.module.css'
...
const FoodCatalogue = () => {
  ...
  useEffect(() => {
    const fetchFoodType = async () => {
      const res = await fetch(`http://localhost:9000/product?category=${foodEndpoint}`, {
        headers: {
          ...
          {filteredFoods.length !== 0 ? filteredFoods.map((f) => (
            <Link to={`/food/${f._id}`} key={f._id} className={classes.food}>
              <div className={classes.imgContainer}>
                <img src={`http://localhost:9000/images/${f?.img}`} className={classes.foodImg} />
              </div>
